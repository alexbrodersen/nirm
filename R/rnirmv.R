...
rnirmv <- function(dataset, nsample, nitem, ndim = 2, 
                   ...
                   pr_mean_w = 0.0, prior_a = 0.001, prior_b = 0.001, option = TRUE, cores = 1, coding = 0){
    ...
    if((niter - nburn) %% nthin == 0){
        output = rnirmvcpp(dataset, nsample, nitem, ndim, niter, nburn, nthin, nprint,
                           ...
                           pr_mean_w, prior_a, prior_b, option, cores, coding)
        ...
        z.star = matrix(NA, nrow = nsample, ncol = ndim)
        for(i in 1:nsample){
            for(j in 1:ndim){            
                z.star[i, j] <- sum((dataset[i, ] * w.star[, j]) / (sum(dataset[i, ]) + .Machine$double.eps))
            }            
        }
        
        z.max_w = output$z[max.address_w,,]
        z.star = procrustes(z.max_w,z.star, translation = TRUE)$X.new
        output$z[max.address_w,,] = z.star
        ...
        for(iter in 1:nmcmc){
            ...
            if(iter != max.address_w) z.proc[iter,,] = procrustes(z.iter,z.star,
                                                                  translation = TRUE)$X.new
            else z.proc[iter,,] = z.iter
            
            w.iter = output$w[iter,,]
            if(iter != max.address_w) w.proc[iter,,] = procrustes(w.iter,w.star,
                                                                  translation = TRUE)$X.new
